...
> [!NOTE]
> We named the iterator variable `i` but we can technically give this any name as long as it is consistent throughout the loop.
...
> [!NOTE]
> When using arrays with loops, you are likely to encounter the notable ["off-by-one" error](https://en.wikipedia.org/wiki/Off-by-one_error) that happens when you're trying to access an index that doesn't exist or if you begin the index with 1 instead of 0.
