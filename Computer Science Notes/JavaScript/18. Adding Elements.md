...
> [!NOTE]
> The first method we encountered was `console.log()` from Chapter 1.
...
> [!NOTE]
> For every new element added to an array with the `.unshift()` method, the indices of the other elements are shifted up by 1.
...
> [!NOTE]
>  By "returns", we mean that `greeks.shift()` produces a single value, the removed first element, that can be used elsewhere in the program (such as being printed or stored in a variable).
