...
But what if we wanted to find out the _index_ of a particular element's value? We can do that with the `.indexOf()` method:
...
> [!NOTE] 
> If there are multiple elements with the same value in an array, the `.indexOf()` returns the index of the first occurrence of that value.
